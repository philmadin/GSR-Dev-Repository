...
<body>
		...
		<article>
			...
			<section id="settings">
				...
				<ul class="grid_8 grid_0" id="settingslist">
					<h6>Settings</h6>
					<li class="opt_tab" id="profpic_button">SHARKFACE (PROFILE PICTURE) <i>&#10094;</i></li>
				...
				<form class="grid_16 grid_0 settingsdisplay" id="profpic" action="updateGSRinfo.php" method="post" enctype="multipart/form-data">
					<ul>
						<li id="sharkface">
							...
							<p>
								<label for="sharkface"><i>Upload a new sharkface picture (MAX 1MB)</i></label>
								<input id="sharkface" name="sharkface" type="file" class="required" accept="image/jpeg" />
							</p>
							<p id="profpic_submit" class="submit_button">
								<br>
								<button name="upload" value="<?php echo $user; ?>" type="submit">UPLOAD</button>
							</p>
						</li>
						<li id="oldimages">
							<h6>Old Profile Pictures</h6>
							<p>
								<?php
									...
									foreach ($OIarr as $img) {
										if(strstr($img, $OIfindA) && strstr($img, $OIfindB)) {
											$altimg = str_replace($OIfindB, "", $img);
											echo "<a class='oldimage' data-old-image='" . $altimg . "' data-user-name='" . $user . "'><img src='imgs/users/" . $img . "' height='135' width='116'></a>";
										}
										if(strstr($img, "default") && strstr($img, $OIfindB)) {
											echo "<a class='oldimage' data-old-image='' data-user-name='" . $user . "'><img src='imgs/users/" . $img . "' height='135' width='116'></a>";
										}
									}
								?>
							</p>
	...
	<script type="text/javascript">
		...
		$(function() {
			...
			if($.urlParam('change_image') == 'filesize_error') {
				$(".opt_tab").removeClass('active');
			...
			} else if($.urlParam('change_image') == 'file_empty') {
				$(".opt_tab").removeClass('active');
				$("#profpic_button").addClass('active');
			...
			} else if($.urlParam('change_image') == 'file_invalid') {
				$(".opt_tab").removeClass('active');
				$("#profpic_button").addClass('active');
			...
			} else if($.urlParam('change_image') == 'upload_successful') {
				$(".opt_tab").removeClass('active');
				$("#profpic_button").addClass('active');
			...
			$(".oldimage").each(function(index, el) {
				$(this).click(function(event) {
					var oldimage = "reimage=reimage&oldimage=" + $(this).attr("data-old-image") + "&oldimageuser=" + $(this).attr("data-user-name");
					$.ajax({
			            url : "updateGSRinfo.php",
			            data : oldimage,
			            type : "GET",
