...
function clean($string) {
   ...
   return preg_replace('/[^A-Za-z0-9\-]/', '', $string); // Removes special chars.
    ...
    if(isset($_POST['submit'])) {
        ...
        $SUBMIT_articletitle    = $_POST['articleopiniontitle'];
	...
	$SUBMIT_content       = htmlentities($SUBMIT_content, ENT_QUOTES); 
        ...
        $main_file          = "texts/opinion/" . clean($SUBMIT_articletitle) . "_main.txt";
		...
		$SUBMIT_articletitle    = mysqli_real_escape_string($con, $_POST['articleopiniontitle']);
        mysqli_query($con, "INSERT INTO tbl_opinion (article_type, title, main, Content, author, authuser, createdate, month, year, pending, beta_approved, alpha_approved, editors_choice, tags) VALUES ('$SUBMIT_articletype', '".mysqli_real_escape_string($con, $SUBMIT_articletitle)."', '$main_file', '$SUBMIT_content', '$SUBMIT_author', '$SUBMIT_username', '$SUBMIT_date', '$SUBMIT_month', '$SUBMIT_year', '$ifbossa', '$ifbossb', 'false', '0', '$SUBMIT_tags')");
	...
	$SUBMIT_content       = htmlentities($SUBMIT_content, ENT_QUOTES); 
		...
		$article_query = mysqli_query($con, "SELECT * FROM tbl_opinion WHERE id = '$SUBMIT_id'");
        ...
        mysqli_query($con, "UPDATE tbl_opinion SET article_type = '$SUBMIT_articletype', title = '$SUBMIT_articletitle', Content = '$SUBMIT_content', tags = '$SUBMIT_tags' WHERE id = '$SUBMIT_id'");
    }
    ...
    if(isset($_POST['upload_images'])) {
        ...
        $article_query = mysqli_query($con, "SELECT * FROM tbl_opinion WHERE id = '$upload_id'");
        ...
        $upload_path = $_SERVER['DOCUMENT_ROOT'] . "/imgs/opinion/";
        ...
        if($aimage_info) {
            ...
            if($imagewidth <= 960) {
                header("Location: articleimages.php?article_images=" . $upload_id . "&upload_error=width&type=opinion");
            }
        } else {
            header("Location: articleimages.php?article_images=" . $upload_id . "&upload_error=invalid&type=opinion");
        }

        if(!empty($UPLOAD_aimage['name'])) {
            $aimage_path = clean("GSR-" . $UPLOAD_articletype . "-" . $UPLOAD_articletitle . "-" . md5($aimage_name)) . "-a_image.jpg";
            $aimage_qury = mysqli_query($con, "UPDATE tbl_opinion SET a_image = '$aimage_path' WHERE id = '$upload_id'");
        ...
        if(!empty($UPLOAD_bimage['name'])) {
            ...
            $bimage_qury = mysqli_query($con, "UPDATE tbl_opinion SET b_image = '$bimage_path' WHERE id = '$upload_id'");
        ...
        if(!empty($UPLOAD_cimage['name'])) {
            ...
            $cimage_qury = mysqli_query($con, "UPDATE tbl_opinion SET c_image = '$cimage_path' WHERE id = '$upload_id'");
        ...
        if(!empty($UPLOAD_dimage['name'])) {
            ...
            $dimage_qury = mysqli_query($con, "UPDATE tbl_opinion SET d_image = '$dimage_path' WHERE id = '$upload_id'");
        ...
        if(!empty($UPLOAD_eimage['name'])) {
            ...
            $eimage_qury = mysqli_query($con, "UPDATE tbl_opinion SET e_image = '$eimage_path' WHERE id = '$upload_id'");
            $eimage_upld = move_uploaded_file($eimage_temp, $upload_path . $eimage_path);
        }

        header("Location: articlelist.php?type=opinion");
    }

    if(isset($_GET['pending'])) { mysqli_query($con, "UPDATE tbl_opinion SET pending = 'true' WHERE id = '" . $_GET['pending'] . "'"); }
    if(isset($_GET['beta'])) { mysqli_query($con, "UPDATE tbl_opinion SET beta_approved = 'true' WHERE id = '" . $_GET['beta'] . "'"); }
    if(isset($_GET['alpha'])) { mysqli_query($con, "UPDATE tbl_opinion SET alpha_approved = 'true' WHERE id = '" . $_GET['alpha'] . "'");
        $SUBMIT_date        = date('Y-m-d H:i:s');
        $SUBMIT_year        = date('Y');
        $SUBMIT_month       = date('M');
        mysqli_query($con, "UPDATE tbl_opinion SET createdate = '$SUBMIT_date', month = '$SUBMIT_month', year = '$SUBMIT_year' WHERE id = '" . $_GET['alpha'] . "'");
	}
...
if(isset($_GET['choice'])) {
    ...
    mysqli_query($con, "INSERT INTO tbl_featured (article_type, article_id, position) VALUES ('Opinion', $newId, $newChoice)");

}

    if(isset($_GET['deny'])) {
        $denyid = $_GET['deny'];
        mysqli_query($con, "UPDATE tbl_opinion SET alpha_approved = 'false' WHERE id = '$denyid'");
        mysqli_query($con, "UPDATE tbl_opinion SET beta_approved = 'false' WHERE id = '$denyid'");
        mysqli_query($con, "UPDATE tbl_opinion SET pending = 'false' WHERE id = '$denyid'");
    }

    if(isset($_GET['deletionid'])) {
        $deletionid = $_GET['deletionid'];

        $files_query = mysqli_query($con, "SELECT * FROM tbl_opinion WHERE id = '$deletionid'");
