...
$totalArts = mysqli_num_rows(mysqli_query($con, "SELECT id FROM tbl_news WHERE authuser = '$user' AND alpha_approved = 'true'"));
...
$query_user_arts = "SELECT id, article_type, title, author, month, a_image, views, createdate, bites FROM tbl_news WHERE authuser = ? AND alpha_approved = 'true' ORDER BY createdate DESC LIMIT 9 OFFSET " .  $offset . ";";
...
if ($totalArts == 0) { // If the user has written no news.
  echo "<br><p class='profile_alert'>This user has written no News</p>";
  $pages = 1;
} else {
