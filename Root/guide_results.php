...
$total_num = mysqli_num_rows(mysqli_query($con, "SELECT * FROM tbl_guide WHERE alpha_approved = 'true'" . $filtered_month . $filtered_year . " ORDER BY id DESC"));
$guide_link_list = mysqli_query($con, "SELECT * FROM tbl_guide WHERE alpha_approved = 'true'" . $filtered_month . $filtered_year . " ORDER BY id DESC LIMIT " . $final_offset . ",10");

while ($rvw_row = mysqli_fetch_assoc($guide_link_list)) {
    $art_name	= $rvw_row['title'];
    $images 	= unserialize($rvw_row['images']);
    $art_file   = $images[0];
