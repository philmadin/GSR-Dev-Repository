...
$tQUERY = mysqli_query($con, "SELECT * FROM tbl_guide WHERE title = '$guide_title'");
if(mysqli_num_rows($tQUERY)==0){ $articleSet = false; }
while ($tROW = mysqli_fetch_array($tQUERY)) {
    $articleid 			= $tROW['id'];
    $articletype 	 	= $tROW['article_type'];
    $title 			 	= $tROW['title'];
    $file_intro			= "http://gamesharkreviews.com/".$tROW['intro'];
    $genre			 	= $tROW['genre'];
    $author			 	= $tROW['author'];
    ...
    <div class="container">
        ...
        <div class="row white-page">
            ...
            <div class="col-lg-12">
                ...
                <img src="http://gamesharkreviews.com/imgs/guide/<?php echo $images[0]; ?>" class="img-responsive img-thumbnail">
                ...
                <p class="content" id="intro_section" style="margin:0px !important;">
                    ...
                    <?php echo file_get_contents($file_intro); ?>
                </p>
                <?php if($checklist!=""){?>
                ...
                <?php } ?>
                <?php $step_num = 0;foreach($steps as $step_file){
                    $step_num++;?>
                    <p class="content well guide-step" id="step-<?php echo $step_num;?>">
                        <span class="step-num">STEP <?php echo $step_num;?></span>
                        <span class="step-image" style="background-image:url(<?php echo 'http://gamesharkreviews.com/imgs/guide/'.$images[$step_num];?>);"></span>
                        <?php echo str_replace("h3", "span class='sub-title'", str_replace("\n","<br>",file_get_contents("http://gamesharkreviews.com/".$step_file))); ?>
                    </p>
                <?php } ?>
...
<script type="text/javascript">
    ...
    $(".social_share_side button, .social_share_bottom button").click(function(e){
        ...
        ae8857b082115f203e8a5d23410461f7(50, articletype, articleid, actionType, "<h3>Article Share!</h3> You have been rewarded %xp%xp<br />for sharing guide/walkthrough.");
    });
    ...
    $(function() {
        ...
        $.post("article_view_action.php",
            ...
            function(data, status){
                ...
                setTimeout(function(){
                    ae8857b082115f203e8a5d23410461f7(150, articletype, articleid, "view", "<h3>Article View!</h3> You have been rewarded %xp%xp<br />for reading this guide/walkthrough.");
                }, 30000);
            });
    });
    ...
    $("#bite").click(function(){
        ...
        $.get("bite_action.php",
            ...
            function(data, status){
                ...
                ae8857b082115f203e8a5d23410461f7(50, articletype, articleid, "bite", "<h3>Article Bite!</h3> You have been rewarded %xp%xp<br />for biting this guide/walkthrough.");
