...
function implode_multi($arr, $join, $key){
    ...
    if(isset($_POST['save'])) {
		...
		$sqlero = mysqli_prepare($con,"INSERT INTO tbl_guide_content (articleidFK, Content) VALUES (?, ?)") or die(mysqli_error($con));
    		mysqli_stmt_bind_param($sqlero, 'is', $SUBMIT_id, $SUBMIT_intro) or die(mysqli_error($con));
   		mysqli_stmt_execute($sqlero) or die(mysqli_error($con));
		foreach($step_content_array as $content){
			$sqlero = mysqli_prepare($con,"INSERT INTO tbl_guide_content (articleidFK, Content) VALUES (?, ?)") or die(mysqli_error($con));
	    		mysqli_stmt_bind_param($sqlero, 'is', $SUBMIT_id, $content) or die(mysqli_error($con));	
    			mysqli_stmt_execute($sqlero) or die(mysqli_error($con));	
		}
    }
...
if(isset($_GET['choice'])) {
    ...
    if(isset($_GET['deletionid'])) {
        ...
        mysqli_query($con, "DELETE FROM tbl_guide_content WHERE articleidFK='$deletionid'");
        mysqli_query($con, "DELETE FROM tbl_guide WHERE id = '$deletionid'");
