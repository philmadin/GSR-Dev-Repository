...
// If the action performed was read a review
if (strcmp($action_type, "Review") == 0) {
  $query = mysqli_fetch_assoc(mysqli_query($con, "SELECT title, gamename FROM tbl_review WHERE id = '$item_id'"));
...
} else { //If the action performed was read an opinion or a guide or a news
  $query = mysqli_fetch_assoc(mysqli_query($con, "SELECT title FROM tbl_" . strtolower($item_type) . " WHERE id = '$item_id'"));
  $title = urlencode(str_replace(" ", "_", $query['title']));
  $url = "review.php?t=" . $title;
}
