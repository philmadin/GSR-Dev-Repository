<?php
	...
	$total_num = mysqli_num_rows(mysqli_query($con, "SELECT * FROM tbl_news WHERE alpha_approved = 'true'" . $filtered_month . $filtered_year . " ORDER BY id DESC"));
	$news_link_list = mysqli_query($con, "SELECT * FROM tbl_news WHERE alpha_approved = 'true'" . $filtered_month . $filtered_year . " ORDER BY id DESC LIMIT " . $final_offset . ",10");

	while ($rvw_row = mysqli_fetch_assoc($news_link_list)) {
